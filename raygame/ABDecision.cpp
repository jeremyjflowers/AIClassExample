...
void ABDecision::makeDecision(Agent* agent, float deltaTime)
{
	...
	{
		//If the function returns true and the left child isn't null, move to the left node
		if (m_leftChild)
			m_leftChild->makeDecision(agent, deltaTime);
	}
	else
	{
		//If the function returns false and the right child isn't null, move to the right node
		if (m_rightChild)
